...
function evolution_figure(){
  ...
  create_letter(left + width - 50,top + height + 30,"レシピ")
	...
	for (let i = 0; i < recipe_power.length-1; i++) {
		...
		recipe_introduce(x1,y1,`introduce_${recipe_id[i]}`,height)
	}
  ...
  const a = document.getElementById('a');
    ...
    //recipe_introduce(x,y,`introduce_${recipe_id[i]}`,height)
...
function recipe_introduce(x,y,id, height ){

  const canvas = document.getElementById('canvas');
  const rect = canvas.getBoundingClientRect();
  const absoluteTop = rect.top + window.pageYOffset;
  const absoluteLeft = rect.left + window.pageXOffset;
  var introduce = document.getElementById(id);
